...
void InterestingPoints::SetInterestingPoints(PointSet newPoints)
{
    myInterestPoints = newPoints;
}
...
ImgMatrix InterestingPoints::HarrisResponse(int windowRadius)
{
    ...
    for (int u = -windowRadius; u <= windowRadius; u++)
    {
        ...
        for (int v = -windowRadius; v <= windowRadius; v++)
        {
            double val = coeff * exp(- (u * u + v * v) / delitel);
            gaussRow.append(val);
            sum += val;
        }
        gaussKernel.append(gaussRow);
    }
    foreach(QVector<double> gRow, gaussKernel)
        foreach(double gVal, gRow)
            gVal /= sum;
